...
public class SetBitbucketBindingAction extends SetBindingAlmSettingsWsAction {
    ...
    public void define(WebService.NewController context) {
        WebService.NewAction action = context.createAction("set_bitbucket_binding").setPost(true).setHandler(this);

        action.createParam(PARAM_ALM_SETTING).setRequired(true);
        action.createParam(PARAM_PROJECT).setRequired(true);
        action.createParam(PARAM_REPOSITORY).setRequired(true);
        action.createParam(PARAM_SLUG).setRequired(true);
    }
        ...
        try (DbSession dbSession = dbClient.openSession(false)) {
            ...
            dbClient.projectAlmSettingDao().insertOrUpdate(dbSession,
                                                           new ProjectAlmSettingDto().setProjectUuid(project.uuid())
                                                                   ...
                                                                   .setAlmRepo(repository).setAlmSlug(slug));
            dbSession.commit();
        }
