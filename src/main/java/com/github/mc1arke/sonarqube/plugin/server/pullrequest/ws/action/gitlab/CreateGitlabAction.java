...
public class CreateGitlabAction extends AlmSettingsWsAction {
    ...
    public void define(WebService.NewController context) {
        WebService.NewAction action = context.createAction("create_gitlab").setPost(true).setHandler(this);
    ...
    public void handle(Request request, Response response) {
        ...
        String accessToken = request.mandatoryParam(PARAM_PERSONAL_ACCESS_TOKEN);

        try (DbSession dbSession = dbClient.openSession(false)) {
            checkAlmSettingDoesNotAlreadyExist(dbSession, key);
            dbClient.almSettingDao().insert(dbSession, new AlmSettingDto().setAlm(GITLAB).setKey(key)
