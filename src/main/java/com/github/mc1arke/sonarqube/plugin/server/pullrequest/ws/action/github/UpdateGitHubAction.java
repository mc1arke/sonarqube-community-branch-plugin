...
public class UpdateGitHubAction extends AlmSettingsWsAction {
    ...
    public void define(WebService.NewController context) {
        WebService.NewAction action = context.createAction("update_github").setPost(true).setHandler(this);
        ...
        try (DbSession dbSession = dbClient.openSession(false)) {
            ...
            dbClient.almSettingDao().update(dbSession,
                                            almSettingDto.setKey(isNotBlank(newKey) ? newKey : key).setUrl(url)
                                                    .setAppId(appId).setPrivateKey(privateKey));
            dbSession.commit();
        }
